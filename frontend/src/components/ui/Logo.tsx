...
export function Logo({ size = 'md', className = '', showText = false }: LogoProps) {
  const sizeClasses = {
    sm: { main: 'h-6 w-6', euro: 'h-4 w-4' },
    md: { main: 'h-8 w-8', euro: 'h-5 w-5' },
    lg: { main: 'h-10 w-10', euro: 'h-7 w-7' },
    xl: { main: 'h-12 w-12', euro: 'h-8 w-8' }
  };
  ...
  return (
    <div className={`flex items-center gap-3 ${className}`}>
      <div className="relative inline-flex items-center justify-center">
        ...
        <Euro className={`${sizeClasses[size].euro} text-[#c5a572] absolute -bottom-2 -right-2 bg-[#162238] rounded-full p-0.5 transition-transform group-hover:scale-110 duration-300`} />
      </div>
      {showText && (
